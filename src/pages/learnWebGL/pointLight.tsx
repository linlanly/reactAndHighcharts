...
function initShader(gl: WebGL2RenderingContext): WebGLShader | null {
  ...
  void main() {
    ...
    vnormal = normalize(vec3(normalmatrix * setsomedata));
    vposition = vec3(modelMat * position);
    vcolor = color;
  }`
  let fsource = `
  ...
  void main() {    
    ...
    vec3 lightDirection = normalize(lightd - vposition);
    float dotl = max(dot(lightDirection, normal), 0.0);
    vec3 diffuse = lightc * vec3(vcolor) * dotl;
    vec3 ambient = envc * vcolor.rgb;
...
export default function baseContent() {
  useEffect(() => {
    ...
    let modelMat = gl.getUniformLocation(program, 'modelMat')

    gl.uniform3f(lightc, 1.0, 0, 0)
    gl.uniform3f(envc, .2, .2, .2)

    let lightDirection = new Vector3([0, 3, 4])
    ...
    modealMa4.rotate(90, false, true, true)
    gl.uniformMatrix4fv(modelMat, false, modealMa4.elements)
